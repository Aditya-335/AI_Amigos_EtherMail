...
export function useAnimateInView(threshold: number) {
  const ref = useRef(null);
  const inView = useInView(ref, { amount: threshold });
  return [ref, inView] as const; // 👈 make sure this is always a ref, never false
}
  ...
  return (
    <motion.div
      ...
      animate={{
        x: [
          initialX,
          `${parseFloat(initialX.toString()) + 50}${
            initialX.toString().includes("vw") ? "vw" : "px"
          }`,
          `${parseFloat(initialX.toString()) - 30}${
            initialX.toString().includes("vw") ? "vw" : "px"
          }`,
          initialX,
        ],
        y: [
          initialY,
          `${parseFloat(initialY.toString()) - 40}${
            initialY.toString().includes("vh") ? "vh" : "px"
          }`,
          `${parseFloat(initialY.toString()) + 60}${
            initialY.toString().includes("vh") ? "vh" : "px"
          }`,
          initialY,
        ],
...
function LandingPage() {
  ...
  const headerShadow = useTransform(
    scrollYProgress,
    [0, 0.05],
    ["0px 0px 0px rgba(0,0,0,0)", "0px 4px 15px rgba(0, 0, 0, 0.05)"]
  );
  ...
  const handleWatchDemoClick = () => {
    window.open("https://drive.google.com/file/d/19NFnO6_99n7fVLOc3RMh2ytdF8E5AGR9/view?usp=sharing");
  };
  ...
  const features = [
    {
      ...
      description: "Crystal-clear summaries of long emails instantly.",
    },
    {
      icon: Send,
      title: "Smart Compose",
      description: "AI crafts replies that sound just like you, only faster.",
    },
    {
      icon: Search,
      title: "Cognitive Search",
      description:
        "Find anything, anytime, understanding context, not just keywords.",
    },
    {
      icon: Star,
      title: "Priority Flow",
      description: "Automatically surfaces what needs your attention now.",
    },
    {
      icon: Clock,
      title: "Predictive Scheduling",
      description: "Suggests the perfect time to send emails for max impact.",
    },
    {
      icon: FileEdit,
      title: "Tone Analysis",
      description:
        "Ensure your message lands perfectly with AI tone suggestions.",
    },
    {
      icon: Shield,
      title: "Quantum Security",
      description: "Next-gen encryption protecting your communications.",
    },
    {
      icon: Zap,
      title: "Hyper Processing",
      description: "Experience truly instant email handling and sorting.",
    },
    {
      icon: Settings,
      title: "Adaptive Workflows",
      description:
        "Your email learns and automates based on your unique habits.",
    },
  ];
    ...
    <div className="min-h-screen font-sans bg-gradient-to-br from-cyan-50 via-white to-purple-50 text-gray-800 overflow-x-hidden relative">
      ...
      <div className="relative z-10">
            ...
            <div className="flex items-center justify-between h-16 md:h-20">
              ...
              <div className="hidden md:flex items-center space-x-8">
                ...
                <motion.button
                  whileHover={{
                    scale: 1.03,
                    boxShadow: "0px 5px 15px rgba(100, 116, 139, 0.2)",
                  }}
                  whileTap={{ scale: 0.97 }}
                ...
                <nav className="flex flex-col space-y-6 mb-auto">
                  {navLinks.map((item) => (
                    <motion.a
                      ...
                      transition={{
                        delay: 0.1 + navLinks.indexOf(item) * 0.05,
                      }}
                      href={item.href}
                      onClick={toggleMobileMenu}
                ...
                <div className="flex flex-col space-y-4 mt-8">
                  ...
                  <motion.button
                    ...
                    whileHover={{
                      scale: 1.03,
                      y: -1,
                      boxShadow: "0 8px 15px -3px rgba(14, 165, 233, 0.3)",
                    }}
                    whileTap={{ scale: 0.97 }}
                    onClick={() => {
                      navigate("/signup");
        ...
        >
          ...
          >
            AetherMail uses cutting-edge AI to streamline your email, automate
            tasks, and give you back your focus. Experience effortless
            communication.
          </motion.p>
          <motion.div
            variants={childVariants}
          ...
          >
            <motion.button
              whileHover={{
                scale: 1.05,
                y: -2,
                boxShadow: "0 12px 25px -8px rgba(14, 165, 233, 0.5)",
              }}
              whileTap={{ scale: 0.95 }}
            ...
            <motion.button
              whileHover={{
                scale: 1.05,
                y: -2,
                boxShadow: "0 8px 20px -8px rgba(107, 114, 128, 0.5)",
              }}
              whileTap={{ scale: 0.95 }}
        ...
        >
          ...
          >
            Go beyond basic email. AetherMail transforms your workflow with
            intelligent features designed for modern communication.
          </motion.p>
            ...
            {features.map((feature, index) => (
              ...
              >
                <motion.div
                  ...
                  className={`w-12 h-12 rounded-lg flex items-center justify-center mb-5 bg-gradient-to-br ${
                    index % 3 === 0
                      ? "from-cyan-100 to-blue-100"
                      : index % 3 === 1
                      ? "from-purple-100 to-pink-100"
                      : "from-teal-100 to-green-100"
                  } transition-colors group-hover:from-cyan-200 group-hover:to-purple-200`}
                >
                  <feature.icon
                    className={`w-6 h-6 ${
                      index % 3 === 0
        ...
        >
          ...
          <div className="relative max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center z-10">
            ...
            <motion.p
              variants={childVariants}
              className="text-lg sm:text-xl text-purple-100 mb-10"
            >
              Join our waitlist for exclusive early access, special offers, and
              updates as we launch AetherMail.
            </motion.p>
            <motion.form
              variants={childVariants}
